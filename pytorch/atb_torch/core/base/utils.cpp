...
void *Utils::CreateWorkspae(uint64_t workspaceSize)
{
    ...
    if (workspaceSize <= KB_1) {
        ...
        tensorDesc.shape.dims[2] = workspaceSize / MB_1 + 1; // translated2translated
    } else {
        tensorDesc.shape.dimNum = 4; // dim=4
        tensorDesc.shape.dims[0] = KB_1;
        tensorDesc.shape.dims[1] = KB_1;
        tensorDesc.shape.dims[2] = KB_1;                     // translated3translated,translated2translated
        tensorDesc.shape.dims[3] = workspaceSize / GB_1 + 1; // translated3translated
    }
