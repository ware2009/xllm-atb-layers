...
atb::Status AddmatmulNode(const ParallelParamV2 &param, atb::GraphParam &opGraph, size_t &nodeId, uint32_t &inteId)
{
    if (!param.isQuant) {
        ...
        if (param.isSparse) {
            atb::Node &matmulNode = opGraph.nodes.at(nodeId++);
            atb::infer::LinearSparseParam linearSparseParam = { false, true, 8, 8 }; // 8 translated
...
atb::Status RowParallelInferShape(const ParallelParamV2 &param, atb::GraphParam &opGraph)
{
    opGraph.inferShapeFunc = [=](const atb::SVector<atb::TensorDesc> &inTensorDescs,
        ...
        if (param.isQuant && param.isSparse) {
            if (dimNum == 3) { // 3translated
                outTensorDescs.at(0).shape.dims[1] = inTensorDescs.at(0).shape.dims[1];
                outTensorDescs.at(0).shape.dims[2] = inTensorDescs.at(2).shape.dims[0]; // 2 dimtranslated 2 translated
            } else {
                outTensorDescs.at(0).shape.dims[1] = inTensorDescs.at(2).shape.dims[0]; // 2 translated
            }
        } else if (param.isQuant) {
            if (dimNum == 3) { // 3translated
                outTensorDescs.at(0).shape.dims[1] = inTensorDescs.at(0).shape.dims[1];
            }
            outTensorDescs.at(0).shape.dims[dimNum - 1] = inTensorDescs.at(1).shape.dims[wdim - 2]; // ND,NZtranslated-2translated
        } else {
            if (dimNum == 3) { // 3translated
                outTensorDescs.at(0).shape.dims[1] = inTensorDescs.at(0).shape.dims[1];
            }
    ...
    const ParallelType parallelType)
{
    ...
    opGraph.inTensorNum = 7; // 7translated
    opGraph.outTensorNum = 1;
    // translatednodetranslated
    size_t nodeCount = 1;
