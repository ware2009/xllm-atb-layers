...
{
    newShape = oldShape;
    if (oldShape.dimNum >= 2) {  // 2: translatedtensortranslated,translated2translated
...
void UnsqueezeAxis(const atb::Dims &oldShape, atb::Dims &newShape, int32_t axis)
{
    ...
    for (uint64_t i = axis + 1; i < std::min(newShape.dimNum, static_cast<uint64_t>(8)); i++) {  // 8: tensortranslated
...
{
    if (oldShape.dimNum == 4) {  // 4: translated[B,S,N,D],translated[BS,ND]
    ...
    const atb::Dims& oldShape, atb::Dims& newShape, int32_t headNum, int32_t kvHeadNum, int32_t headDim)
{
    ...
    newShape.dimNum = 4;  // 4: translatedshapetranslated4
