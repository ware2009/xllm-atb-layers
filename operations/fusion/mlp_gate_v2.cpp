    ...
    atb::GraphParam &opGraph)
{
    if (!param.noGate) {
        if (param.isPack) {
            ...
            splitParam.splitNum = 2;  // 2: translated
...
atb::Status MlpGateLayerV2(const MlpGateParamV2 &param, atb::Operation **operation)
{
    ...
    opGraph.inTensorNum = 19; // 19translated
    opGraph.outTensorNum = 1;
    size_t interTensorNum = 0;
    if (param.noGate) {
        interTensorNum = 2; // 2translated
    } else if (param.isPack) {
        interTensorNum = 5; // 5translated
    } else {
        interTensorNum = 4; // 4translated
    }
