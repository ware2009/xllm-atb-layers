...
namespace atb_speed {
namespace common {
atb::Status InnerInferShape(const ParallelLmHeadParam &param, atb::GraphParam &opGraph)
{
    if (param.gatherAhead) {
        opGraph.inferShapeFunc = [=](const atb::SVector<atb::TensorDesc> &inTensorDescs,
            ...
            outTensorDescs.at(0).shape.dims[0] = inTensorDescs.at(2).shape.dims[0]; // 2 translated
...
atb::Status CreateParallelLmHeadBase(const ParallelLmHeadParam &param, atb::Operation **operation, T config)
{
    ...
    if (param.rankSize > 1) {
        ...
        if (param.unpadInputs) {
            transposeParam.perm = { 1, 0, 2 }; // 2 translated
        } else {
            transposeParam.perm = { 1, 2, 0, 3 }; // 2 3 translated
        }
    ...
    CHECK_OPERATION_STATUS_RETURN(InnerInferShape(param, opGraph));
...
atb::Status ParallelLmHead(const ParallelLmHeadParam &param, atb::Operation **operation)
{
    if (param.rankSize > 1) {
        if (!param.gatherAhead) {
            return CreateParallelLmHeadBase(param, operation,
                ParallelLmHeadConfig(2, 1, 1, 2)); // 2 translated 1 translated 1 translated 2 translated translated
        } else if (param.unpadInputs) {
            return CreateParallelLmHeadBase(param, operation,
                ParallelLmHeadGatherAheadConfig(3, 1, 2, 3)); // 3 translated 1 translated 2 translated 3 translated translated
        } else {
            ATB_SPEED_LOG_ERROR("[gatherAhead] can only used with [unpadInputs]");
            return atb::ERROR_INVALID_PARAM;
        }
    } else {
        if (!param.gatherAhead) {
            return CreateParallelLmHeadBase(param, operation,
                ParallelLmHeadConfig(2, 1, 0, 1)); // 2 translated 1 translated 0 translated 1 translated translated
        } else if (param.unpadInputs) {
            return CreateParallelLmHeadBase(param, operation,
                ParallelLmHeadGatherAheadConfig(3, 1, 1, 2)); // 3 translated 1 translated 1 translated 2 translated translated
        } else {
            ATB_SPEED_LOG_ERROR("[gatherAhead] can only used with [unpadInputs]");
            return atb::ERROR_INVALID_PARAM;
        }
