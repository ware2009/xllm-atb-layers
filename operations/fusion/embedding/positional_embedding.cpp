...
atb::Status PositionalEmbeddingGather(atb::Operation **operation)
{
    ...
    opGraph.inferShapeFunc = [=](const atb::SVector<atb::TensorDesc> &inTensorDescs,
        ...
        if (inTensorDescs.at(IN_COS_TABLE).shape.dimNum >= 3) {  // 3: translatedIN_COS_TABLEtranslated3
            outTensorDescs.at(0).shape.dimNum = 3;  // 3: translatedtensortranslated3
        } else {
            outTensorDescs.at(0).shape.dimNum = 2;  // 2: translatedtensortranslated2
        }
        outTensorDescs.at(0).shape.dims[0] = 1;
        // unpadInputs=Truetranslated,for looptranslated;unpadInputs=Falsetranslated,for looptranslated,translatedbsztranslatedseqLentranslated
  ...
  opGraph.inferShapeFunc =
      [=](const atb::SVector<atb::TensorDesc>& inTensorDescs,
        ...
        if (inTensorDescs.at(IN_COS_TABLE).shape.dimNum >=
            3) {  // 3: translatedIN_COS_TABLEtranslated3
          outTensorDescs.at(0).shape.dimNum =
              3;  // 3: translatedtensortranslated3
        } else {
          outTensorDescs.at(0).shape.dimNum =
              2;  // 2: translatedtensortranslated2
        }
        outTensorDescs.at(0).shape.dims[0] = 1;
        // unpadInputs=Truetranslated,for
        // looptranslated;unpadInputs=Falsetranslated,for
        // looptranslated,translatedbsztranslatedseqLentranslated
...
int64_t AddCatLV(atb::GraphParam &opGraph, const RotaryPositionEmbeddingParam &param)
{
    ...
    if (!param.isFA) {
        ...
        cat1Node.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
            UnsqueezeHeadNumHeadDim(oldShape, newShape, param.headNum, param.headDim / 2);  // 2: headDimtranslated,translated2
        };
    }
    ...
    if (!param.isFA) {
        ...
        cat2Node.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
            UnsqueezeHeadNumHeadDim(oldShape, newShape, param.kvHeadNum, param.headDim / 2);  // 2: headDimtranslated,translated2
        };
    }
