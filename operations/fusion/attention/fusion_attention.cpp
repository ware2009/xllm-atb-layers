...
std::map<std::string, std::vector<std::string>> GetAttnIntermediateTensorCandidatesPrefill()
{
    std::map<std::string, std::vector<std::string>> attnIntermediateTensorCandidates = {
        {"default",
            {"intermediate_q", "intermediate_self_attention"}
        },
        {"kv_quant_scale",
            {"intermediate_k_int8", "intermediate_v_int8"}
        },
    ...
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap)
{
    ...
    if (param.isPrefill && param.enableXattention) {
        qkvOutTensorNames = {"intermediate_q", "in_decode_k_cache", "in_decode_v_cache"};
    } else {
    ...
    if (isK) {
        if (param.isPrefill && param.enableXattention) {
            kvValueQuantNode.inTensorIds = {
                GetTensorIdx(tensorMap, "in_decode_k_cache"), GetTensorIdx(tensorMap, "in_k_quant_scale"),
        ...
        if (param.isPrefill && param.enableXattention) {
            kvValueQuantNode.inTensorIds = {
                GetTensorIdx(tensorMap, "in_decode_v_cache"), GetTensorIdx(tensorMap, "in_v_quant_scale"),
...
template <typename NormParamType>
atb::Status AddQKVQuantNode(const FusionAttentionParam<NormParamType> &param, atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap, std::string nodeType)
{
