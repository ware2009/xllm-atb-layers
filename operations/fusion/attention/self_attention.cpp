    ...
    if (!param.enableRopeQuantKvcache) {
        if (!param.isFA) {  // Paged Attention path
            if (param.enableXattention && !param.isPrefill) {
                ...
                CHECK_OPERATION_STATUS_RETURN(AddDecodeKVCacheOperation(opGraph, param, tensorMap));
    ...
    if (param.isFA && !param.enableXattention) {  // FA
        ...
        if (param.isPrefill && !param.enableSplitFuse) {  // PA Prefill
            ...
            if (param.enableXattention) {
                CHECK_OPERATION_STATUS_RETURN(ConstructDecodeKVCacheNode(opGraph, param, tensorMap));
                return atb::NO_ERROR;
            } else {    
...
template <typename NormParamType>
int64_t AddDecodeKVCacheOperation(
    atb::GraphParam& opGraph, const FusionAttentionParam<NormParamType>& param,
    std::map<std::string, uint32_t>& tensorMap)
{
    ...
    reshapeDecodeKVCacheNode.operation = new atb_speed::common::ReshapeDecodeKvCacheOperation(
        "ReshapeDecodeKVCacheNode", reshapeDecodeKVCacheParam);
    reshapeDecodeKVCacheNode.inTensorIds = {
        GetTensorIdx(tensorMap, "in_k_cache"), GetTensorIdx(tensorMap, "in_v_cache"),
        GetTensorIdx(tensorMap, "intermediate_k"), GetTensorIdx(tensorMap, "intermediate_v"),
        GetTensorIdx(tensorMap, "in_block_tables"), GetTensorIdx(tensorMap, "in_current_round")
    };
    ...
    std::map<std::string, uint32_t>& tensorMap)
{
    ...
    CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(param.selfAttentionParam, &selfAttentionNode.operation));
    // if use for beam search, the intermediate_k and intermediate_v are the prefill key and value
    ...
    std::map<std::string, uint32_t>& tensorMap)
{
    ...
    if (!param.isPrefill && param.enableAclGraphPagedAttention) {
      atb::customize::CustomPagedAttentionParam customPagedAttentionParam(param.pageAttentionParam);
      CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(customPagedAttentionParam, &selfAttentionNode.operation));
    } else {
    ...
    if (!param.isPrefill && param.enableAclGraphPagedAttention) {
        selfAttentionNode.inTensorIds.push_back(GetTensorIdx(tensorMap, "paged_attention_tiling_data"));
    }
    return atb::NO_ERROR;
}

template <typename NormParamType>
int64_t ConstructDecodeKVCacheNode(
    atb::GraphParam& opGraph, const FusionAttentionParam<NormParamType>& param,
    std::map<std::string, uint32_t>& tensorMap)
{   
    ...
    xAttentionNode.inTensorIds = {
        GetTensorIdx(tensorMap, "intermediate_q"), GetTensorIdx(tensorMap, "in_decode_k_cache"),
        GetTensorIdx(tensorMap, "in_decode_v_cache"), 
        GetTensorIdx(tensorMap, "in_k_cache"), 
        GetTensorIdx(tensorMap, "in_v_cache"),
        GetTensorIdx(tensorMap, "in_block_tables"),
