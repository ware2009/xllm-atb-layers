...
enum AttnInTensorCategory : unsigned int {
  ...
  ATTN_FC,
  /// Tensors required by x attention
  ATTN_X_ATTENTION,
  /// Tensors required by acl graph
  ATTN_ACL_GRAPH,
...
/// \tparam Types of the normalization parameters. Avaliable options are
/// `atb::infer::RmsNormParam` and ``atb::infer::LayerNormParam`.
template <typename NormParamType> struct FusionAttentionParam {
  // QKV linear param
  /// A flag indicating whether the model structure is grouped query attention
  /// or multi head attention
  bool isGroupedQueryAttention = false;
  /// When `isBF16` is true, bfloat16 precision is used; otherwise, float16
  /// precision is used.
  bool isBF16 = false;
  /// A flag indicating whether to reshape before spliting the packed output of
  /// the qkv linear operation
  ...
  /// The backend of the attention module; refer to `OpBackend` for the
  /// supported values
  int attnBackend = atb_speed::common::OpBackend::ATB;
  /// The group size used for dequantizing the weight tensor in the per-group
  ...
  /// q linear, k linear, v linear, dense linear, gate linear, up linear, and
  /// down linear.
  ...
  std::vector<int> layerLinearDescs = {
      common::LinearDesc::INVALID_DESC, common::LinearDesc::INVALID_DESC,
      common::LinearDesc::INVALID_DESC, common::LinearDesc::INVALID_DESC,
      common::LinearDesc::INVALID_DESC, common::LinearDesc::INVALID_DESC,
      common::LinearDesc::INVALID_DESC};
  /// Defines the transpose type of the second matrix in the matmul operation
  /// for the following linear module: q linear, k linear, v linear, dense
  /// linear, gate linear, up linear, and down linear.
  ...
  /// If `isFA` is true, Flash Attention is used; otherwise, Paged Attention is
  /// used
  bool isFA = true;
  /// If `isFIA` is true, the prefill attention uses the fused infer attention
  /// path.
  bool isFIA = false;
  ...
  /// Parameters for the attention operation from the AclNN backend (used in the
  /// decode phase)
  ...
  /// Parameters for the flash attention score operation from the AclNN backend
  atb_speed::common::AclnnFAScoreParam aclnnFAScoreParam;
  std::shared_ptr<int> bs = std::make_shared<int>(0);
  std::shared_ptr<int> seqlen = std::make_shared<int>(0);
  // self out linear param
  /// A flag indicating whether dense linear has bias
  bool selfAttnHasBias = false;
  /// A flag that indicates whether low-latency computation over communication
  /// is enabled
  bool supportLcoc = false;
  bool enableMC2 = false;
  /// The quantization type of the dense linear. Refer to `PackQuantType` in the
  /// `operations/utils.h`.
  ...
  /// A flag indicating whether to use the unified decode kv cache pipeline
  /// (prefill writes kv cache + decode consumes decode kv cache).
...
/// It consists of QKVLinearSplit operation, Rope operation, Elementwise
/// operation to quant intermediate kv tensors, ReshapeAndCache operation,
/// SelfAttention/PageAttention operation and LinearParallel operation for the
/// dense linear.
///
/// \tparam NormParamType Types of the normalization parameters. Avaliable
/// options are `atb::infer::RmsNormParam` and `atb::infer::LayerNormParam`.
...
/// \return A flag indicating whether the operation has been successfully
/// created.
///
/// Operation's inputs:
/// Name               | Requirements | Dtype            | Shape | Description |
/// -------------------|--------------|------------------|-------|----------|
/// in_input           | Required     | float16/bfloat16 | `isFA` is false:
/// [len(all_seq),num_heads,head_dim] | Hidden States | ^                  | ^
/// | ^                | `isFA` is true: [bsz,seq_len,num_heads,head_dim]  | ^ |
/// in_norm_weight     | ^            | Refer to
/// `atb_speed::common::QKVLinearSplit` in the
/// `operations/fusion/attention/qkv_linear_split.h` for more details. |||
/// in_norm_bias       | ^            | ^ |||
/// in_norm_new_weight | ^            | ^ |||
/// in_norm_new_bias   | ^            | ^ |||
/// in_weight_0        | ^            | ^ |||
...
/// in_bias_2          | ^            | ^ |||
/// in_compress_idx_2  | ^            | ^ |||
/// in_cos_embed       | ^            | float16/bfloat16 |
/// [len(all_seq),head_dim] | The cosine part of the rotary embedding. |
/// in_sin_embed       | ^            | ^                | ^ | The sine part of
/// the rotary embedding. | in_seq_len         | ^            | int32 |
/// [batch_size] | The total number of input and output tokens. <br> In the
/// prefill phase, each elements equals to the length of the prompt. <br> For
/// flash attention, each element is set to 1 in the decode phase. <br> For
/// paged attention, each element is set to the number of input tokens plus
/// output tokens in the decode phase. | in_k_cache         | ^            |
/// Refer to `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_v_cache         | ^            | ^ |||
/// in_attention_mask  | ^            | ^ |||
/// in_token_offset    | ^            | ^ |||
/// in_layer_id        | ^            | ^ |||
...
/// in_weight_dense    | ^            | Weights for the dense linear. Refer to
/// `atb_speed::common::LinearParallel` in the
/// `operations/fusion/linear/linear_parallel.h` for more details. |||
/// in_scale_dense     | ^            | ^ |||
/// in_offset_dense    | ^            | ^ |||
/// in_descale_dense   | ^            | ^ |||
/// in_bias_dense      | ^            | ^ |||
/// in_compress_idx_dense | ^         | ^ |||
/// in_slopes          | `param.selfAttentionParam.maskType` is in one of
/// `atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS`,
/// `atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS_SQRT` and
/// `atb::infer::SelfAttentionParam::MASK_TYPE_ALIBI_COMPRESS_LEFT_ALIGN` |
/// Refer to `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_batch_wins      | `param.pageAttentionParam.compressType` equals to
/// `atb::infer::PagedAttentionParam::CompressType::COMPRESS_TYPE_KVHEAD` or
/// `atb::infer::PagedAttentionParam::CompressType::COMPRESS_TYPE_KVHEAD_ROPE` |
/// ^ ||| in_ra_seq_len      | ^            | ^ ||| in_pffset_index    |
/// `param.pageAttentionParam.compressType ==
/// atb::infer::PagedAttentionParam::CompressType::COMPRESS_TYPE_KVHEAD_ROPE` |
/// ^ ||| in_ra_offset       | ^            | ^ ||| in_reshape_seq_len | ^ | ^
/// ||| in_q_len           | `param.pageAttentionParam.calcType ==
/// atb::infer::PagedAttentionParam::CalcType::CALC_TYPE_SPEC` | ^ |||
/// in_k_quant_scale   | `param.pageAttentionParam.quantType ==
/// atb::infer::PagedAttentionParam::QuantType::TYPE_DEQUANT_FUSION` |
/// float16/bfloat16 | [head_num * head_dim] | | in_k_dequant_scale | ^ | Refer
/// to `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_v_quant_scale   | ^            | float16/bfloat16 | [head_num * head_dim]
/// | | in_v_dequant_scale | ^            | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_k_quant_offset  | `param.pageAttentionParam.quantType ==
/// atb::infer::PagedAttentionParam::QuantType::TYPE_DEQUANT_FUSION` and
/// `param.pageAttentionParam.hasQuantOffset` is true | int8 | [head_num *
/// head_dim] | | in_k_dequant_offset| ^            | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_v_quant_offset  | ^            | int8 | [head_num * head_dim] | |
/// in_v_dequant_offset| ^            | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_q_quant_scale   | `param.pageAttentionParam.quantType ==
/// atb::infer::PagedAttentionParam::QuantType::TYPE_QUANT_QKV_ONLINE` | float16
/// | [head_num,head_dim] | | in_k_quant_scale   | ^            | float16 |
/// [kv_head_num,head_dim] | | in_v_quant_scale   | ^            | float16 |
/// [kv_head_num,head_dim] | | in_qk_descale      | ^            | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// q_offset           | ^            | int8 | [head_num,head_dim] | |
/// kv_offset          | ^            | int8 | [kv_head_num,head_dim] | |
/// fa3_v_quant_scale  | ^            | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// fa3_offset         | ^            | int32 | [head_num] | |
/// in_im_mask         | `param.supportLora` is true and `param.useImMask` is
/// true | Refer to atb_speed::common::QKVLinearSplit in the
/// `operations/fusion/attention/qkv_linear_split.h` for more details. |||
...
/// in_dense_lora_a    | ^            | ^ |||
/// in_dense_lora_b    | ^            | ^ |||
/// in_reduce_quant_scale | `param.selfOutLinearTensorParallelInfo.quantType !=
/// atb::infer::AllReduceParam::QuantType::QUANT_TYPE_UNDEFINED` | Refer to
/// `atb_speed::common::LinearParallel` in the
/// `operations/fusion/linear/linear_parallel.h` for more details. |||
/// in_reduce_quant_offset| ^         | ^ |||
/// in_gather_quant_scale | ^         | ^ |||
/// in_gather_quant_offset| ^         | ^ |||
/// in_log_n_scale     | `param.pageAttentionParam.scaleType ==
/// atb::infer::PagedAttentionParam::ScaleType::SCALE_TYPE_LOGN` | Refer to
/// `atb_speed::common::AddSelfAttention` in the
/// `operations/fusion/attention/self_attention.h` for more details. |||
/// in_q_norm_weight   | `param.useQKNorm` is true | Refer to
/// atb_speed::common::QKVLinearSplit in the
/// `operations/fusion/attention/qkv_linear_split.h` for more details. |||
/// in_k_norm_weight   | ^            | ^ |||
...
/// fusionAttentionNode.inTensorIds = {...};  // Passing inputs for the
/// operation in order fusionAttentionNode.outTensorIds = {...};  // Tensor
/// index for out atb::GraphParam opGraph;
