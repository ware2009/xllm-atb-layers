 ...
 */
 ...
 #include "reshape_decode_kv_cache_operation.h"
 #include "aclnn_cache_unshared_kv.h"
 ...
 ReshapeDecodeKvCacheOperation::ReshapeDecodeKvCacheOperation(
     const std::string &name,
     AclNNReshapeDecodeKvCacheParam param) : AclNNOperation(name), param_(param) {}
 
 ReshapeDecodeKvCacheOperation::~ReshapeDecodeKvCacheOperation()
 {
     ATB_SPEED_LOG_DEBUG("ReshapeDecodeKvCacheOperation deconstructor");
     this->DestroyOperation();
 }
 
 uint32_t ReshapeDecodeKvCacheOperation::GetInputNum() const
 {
     uint32_t inputNum = 6;
     return inputNum;
 }
 
 uint32_t ReshapeDecodeKvCacheOperation::GetOutputNum() const
 {
     return DIM2;
 }
 
 atb::Status ReshapeDecodeKvCacheOperation::InferShape(const atb::SVector<atb::TensorDesc> &inTensorDescs,
                                         atb::SVector<atb::TensorDesc> &outTensorDescs) const
 {
     ...
     outTensorDescs.at(1).shape.dimNum = inTensorDescs.at(1).shape.dimNum;
     for (uint32_t i = 0; i < inTensorDescs.at(1).shape.dimNum; ++i) {
         outTensorDescs.at(1).shape.dims[i] = inTensorDescs.at(1).shape.dims[i];
     }
 ...
 int ReshapeDecodeKvCacheOperation::CreateAclNNInTensorVariantPack(const atb::VariantPack &variantPack)
 {
 ...
 int ReshapeDecodeKvCacheOperation::CreateAclNNOutTensorVariantPack(const atb::VariantPack &variantPack)
 {
     AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
 ...
 int ReshapeDecodeKvCacheOperation::CreateAclNNVariantPack(const atb::VariantPack &variantPack)
 {
 ...
 int ReshapeDecodeKvCacheOperation::SetAclNNWorkspaceExecutor()
 {
     ATB_SPEED_LOG_DEBUG(opName_ << " SetAclNNWorkspaceExecutor start");
     AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
     int ret = aclnnCacheUnsharedKvGetWorkspaceSize(
 ...
 int ReshapeDecodeKvCacheOperation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
 {
     ATB_SPEED_LOG_DEBUG(opName_ << " aclnnAddmm start");
    int ret = aclnnCacheUnsharedKv(
