 ...
 */
#include "moe_grouped_matmul_swiglu_operation.h"
#include <cstring>
#include <iostream>
#include <securec.h>
#include <sstream>
...
MoeGroupedMatmulSwigluQuantOperation::MoeGroupedMatmulSwigluQuantOperation(
    const std::string &name,
    AclNNMoeGroupedSwigluMatmulParam param) : AclNNOperation(name), param_(param) {
}

MoeGroupedMatmulSwigluQuantOperation::~MoeGroupedMatmulSwigluQuantOperation() {
}

atb::Status MoeGroupedMatmulSwigluQuantOperation::InferShape(
    const atb::SVector<atb::TensorDesc> &inTensorDescs, atb::SVector<atb::TensorDesc> &outTensorDescs) const
{
    ATB_SPEED_LOG_DEBUG(opName_ << "MoeGroupedMatmulSwigluQuantOperation infer shape start");
    ...
    ATB_SPEED_LOG_DEBUG(opName_ << "MoeGroupedMatmulSwigluQuantOperation infer shape end");
    return 0;
}

uint32_t MoeGroupedMatmulSwigluQuantOperation::GetInputNum() const
{
    return INPUT_NUM;
}

uint32_t MoeGroupedMatmulSwigluQuantOperation::GetOutputNum() const
{
    return OUTPUT_NUM;
}

atb::Dims SetMoeWeightStorageShape(const atb::TensorDesc& atbTensorDesc)
{
    ...
    if (atbTensorDesc.format == ACL_FORMAT_FRACTAL_NZ) {
        ...
        storageTensorDims.dims[2] = 1 + ((atbTensorDesc.shape.dims[1] - 1) / 16);  // 1, 16:2: translated, 16: paddingtranslated
        storageTensorDims.dims[1] = 1 + ((atbTensorDesc.shape.dims[2] - 1) / 32);  // 2, 32:2: translated, 32: paddingtranslated
        storageTensorDims.dims[3] = 16;  // 3, 16:NZtranslated
        storageTensorDims.dims[4] = 32;  // 4, 32:NZtranslated
    }
    return storageTensorDims;
}

atb::Status MoeGroupedMatmulSwigluQuantOperation::CreateAclNNInTensorVariantPack(const atb::VariantPack &variantPack)
{
    ...
    for (size_t i = 0; i < INPUT_NUM; i++) {
        ...
        // StorageShape
        atb::Dims storageTensorDims = SetMoeWeightStorageShape(squeezedAtbTensor.desc);

        // ViewShape and Stride
        atb::Dims viewDims = squeezedAtbTensor.desc.shape;
        if (squeezedAtbTensor.desc.shape.dimNum >= 3 && this->param_.transposeB) {  // 3: translated
            aclnnTensor->strides = GetTransposeTensorStride(viewDims);
            viewDims.dims[0] = squeezedAtbTensor.desc.shape.dims[0];
            viewDims.dims[1] = squeezedAtbTensor.desc.shape.dims[2];  // 1, 2: translated
            viewDims.dims[2] = squeezedAtbTensor.desc.shape.dims[1];  // 1, 2: translated
        } else {
...
atb::Status MoeGroupedMatmulSwigluQuantOperation::CreateAclNNOutTensorVariantPack(const atb::VariantPack &variantPack)
{
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
...
int MoeGroupedMatmulSwigluQuantOperation::SetAclNNWorkspaceExecutor()
{
    ATB_SPEED_LOG_DEBUG(opName_ << " SetAclNNWorkspaceExecutor start");
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
    int ret = aclnnMoeGroupedMatmulSwigluQuantGetWorkspaceSize(aclnnVariantPack.aclInTensors.at(DIM0)->tensor,  // 0: x
...
int MoeGroupedMatmulSwigluQuantOperation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
    ATB_SPEED_LOG_DEBUG(opName_ << " aclnnGroupedMatmul start");
    int ret = aclnnMoeGroupedMatmulSwigluQuant(
