...
#include "aclnn_matmul_operation.h"

namespace atb_speed {
namespace common {

AclnnMatmulOperation::AclnnMatmulOperation(
...
atb::Status AclnnMatmulOperation::InferShape(const atb::SVector<atb::TensorDesc> &inTensorDescs,
                                        atb::SVector<atb::TensorDesc> &outTensorDescs) const
{
    ...
    int nDim = param_.transposeB ? DIM0 : DIM1; // inTensorDescs.at(DIM1).shape.dimNum translated 2
...
uint32_t AclnnMatmulOperation::GetInputNum() const
{
    uint32_t inputNum = DIM2;
    return inputNum;
}

uint32_t AclnnMatmulOperation::GetOutputNum() const
{
    return DIM1;
}

int AclnnMatmulOperation::CreateAclNNVariantPack(const atb::VariantPack &variantPack)
{
...
atb::Dims AclnnMatmulOperation::GetWeightStorageShape(const atb::TensorDesc atbTensorDesc) const
{
    ...
    if (atbTensorDesc.format == ACL_FORMAT_FRACTAL_NZ) {
        // nztranslated (k, n) => (n / 16, k / 16, 16, 16)
        // nztranslated (n, k) => (k / 16, n / 16, 16, 16)
        storageTensorDims.dimNum = NUM4;  // 4translated
...
int AclnnMatmulOperation::CreateAclNNInTensorVariantPack(const atb::VariantPack &variantPack)
{
...
int AclnnMatmulOperation::CreateAclNNOutTensorVariantPack(const atb::VariantPack &variantPack)
{
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
...
int AclnnMatmulOperation::SetAclNNWorkspaceExecutor()
{
    ATB_SPEED_LOG_DEBUG(opName_ << " SetAclNNWorkspaceExecutor start");
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;

    int ret = aclnnMatmulGetWorkspaceSize(
...
int AclnnMatmulOperation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
    ATB_SPEED_LOG_DEBUG(opName_ << " aclnnMatmul start");
    int ret = aclnnMatmul(
