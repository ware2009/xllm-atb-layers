...
#include "moe_init_routing_operation_v3.h"

namespace atb_speed {
namespace common {

MoeInitRoutingV3Operation::MoeInitRoutingV3Operation(
    const std::string &name, MoeInitRoutingV3Param param) : AclNNOperation(name), param_(param) {}
MoeInitRoutingV3Operation::~MoeInitRoutingV3Operation() {}

atb::Status MoeInitRoutingV3Operation::InferShape(
    const atb::SVector<atb::TensorDesc> &inTensorDescs, atb::SVector<atb::TensorDesc> &outTensorDescs) const
{
    ATB_SPEED_LOG_DEBUG(opName_ << "MoeInitRoutingV3Operation infer shape start");

    outTensorDescs.at(DIM0).format = inTensorDescs.at(DIM0).format;
    if (param_.quantMode == -1){
    ...
    outTensorDescs.at(DIM2).dtype = aclDataType::ACL_INT64;
    if (param_.expertTokensNumType == 2){
        outTensorDescs.at(DIM2).shape.dimNum = DIM2;
    } else {
        outTensorDescs.at(DIM2).shape.dimNum = DIM1;
    }
    outTensorDescs.at(NUM3).format = inTensorDescs.at(DIM0).format;
    outTensorDescs.at(NUM3).dtype = aclDataType::ACL_FLOAT;
    outTensorDescs.at(NUM3).shape.dimNum = DIM1;

    ATB_SPEED_LOG_DEBUG(opName_ << "MoeInitRoutingV3Operation infer shape origin \
    ...
    if (param_.expertTokensNumType == 2){
        outTensorDescs.at(DIM2).shape.dims[DIM1] = 2;
    }
    outTensorDescs.at(NUM3).shape.dims[DIM0] = inTensorDescs.at(DIM0).shape.dims[DIM0] * scaledTopk;

    ATB_SPEED_LOG_DEBUG(opName_ << "MoeInitRoutingV3Operation infer shape end");
    return 0;
}
uint32_t MoeInitRoutingV3Operation::GetInputNum() const
{
    return DIM2;
}

uint32_t MoeInitRoutingV3Operation::GetOutputNum() const
{
    return NUM4;
}

int MoeInitRoutingV3Operation::SetAclNNWorkspaceExecutor()
{
    int scaledTopk = param_.enableInitRoutingCutoff ? param_.scaledTopk : param_.topkNum;
    ATB_SPEED_LOG_DEBUG(opName_ << " MoeInitRoutingV3Operation start");
    AclNNVariantPack &aclnnVariantPack = this->aclnnOpCache_->aclnnVariantPack;
    int ret = aclnnMoeInitRoutingV3GetWorkspaceSize(
        ...
        0, param_.expertNum, 0, param_.expertTokensNumType, true, param_.quantMode,
        aclCreateIntArray(new int64_t[2]{0LL, param_.expertNum}, 2), 0,
...
int MoeInitRoutingV3Operation::ExecuteAclNNOp(uint8_t *workspace, aclrtStream &stream)
{
    ATB_SPEED_LOG_DEBUG(opName_ << " MoeInitRoutingV3Operation start");

    int ret = aclnnMoeInitRoutingV3(
        workspace, this->aclnnOpCache_->workspaceSize, this->aclnnOpCache_->aclExecutor, stream);
    ATB_SPEED_LOG_DEBUG(opName_ << " MoeInitRoutingV3Operation end, ret:" << ret);
    return ret;
}
