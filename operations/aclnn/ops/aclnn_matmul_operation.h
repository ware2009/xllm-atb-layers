...
struct AclnnMatmulParam {
    bool transposeB = true; // atb LinearParam transposeB translated true
    aclDataType outDataType = ACL_FLOAT16;
};

class AclnnMatmulOperation : public AclNNOperation {
public:
    explicit AclnnMatmulOperation(const std::string &name, AclnnMatmulParam param);
    ~AclnnMatmulOperation() override;
