...
GetQwen2_5LayerTensorCandidates() {
  std::map<std::string, std::vector<std::string>> qwenLayerTensorCandiadates = {
      {"default_weight",
       {"in_input_norm_weight", "in_post_attn_norm_weight", "in_qkv_weight",
        "in_qkv_bias", "in_attn_proj_weight", "in_attn_proj_bias",
        "in_mlp_gate_weight", "in_mlp_gate_bias", "in_mlp_up_weight",
        "in_mlp_up_bias", "in_mlp_down_weight", "in_mlp_down_bias",
        "in_q_weight", "in_q_bias", "in_k_weight", "in_k_bias", "in_v_weight",
        "in_v_bias"}},
      {"default_input",
       {
           "in_hidden_states",
      ...
      {"intermediate_out",
       {"input_norm_out", "qkv_linear_out", "intermediate_q", "intermediate_k",
        "intermediate_v", "intermediate_rope_k", "intermediate_rope_q",
        "intermediate_atten_out", "intermediate_proj_out",
        "intermediate_add_out", "intermediate_post_atten_norm_out",
        "intermediate_gateup_out", "intermediate_silu_out",
        "intermediate_down_out"}},
      {"tensor_parallel", {"allreduce_out_proj", "allreduce_down"}},
...
ConstructTensorMap(const VisionEncoderLayerParam &param, uint32_t &inTensorNum,
  ...
  atb_speed::common::AddTensorToList(qwenLayerTensorCandiadates,
                                     "default_weight", inTensorList);
  // input
  atb_speed::common::AddTensorToList(qwenLayerTensorCandiadates,
                                     "default_input", inTensorList);
  // out
  atb_speed::common::AddTensorToList(qwenLayerTensorCandiadates, "out",
                                     outTensorList);
  // intermediate
  ...
  if (param.worldSize > 1) {
    ...
    atb_speed::common::AddTensorToList(
        qwenLayerTensorCandiadates, "tensor_parallel", intermediateTensorList);
  }
...
atb::Status
BuildAttentionBlock(const VisionEncoderLayerParam &param,
                    const std::map<std::string, uint32_t> &tensorMap,
                    atb::GraphParam &opGraph, bool isTp) {
  ...
  ropeNode.inTensorIds = atb_speed::common::GetTensorIdxList(
  ...
  selfAttentionNode.inTensorIds = atb_speed::common::GetTensorIdxList(
      tensorMap, {"intermediate_rope_q", "intermediate_rope_k",
  ...
  if (isTp) {
    ...
    CreateLinearOperation(&outProjNode.operation, false, true);
  ...
  // Tp Bias Add
  if (isTp) {
    CreateAllReduceOperation(param, &allReduceNode.operation);
    allReduceNode.inTensorIds = atb_speed::common::GetTensorIdxList(
        tensorMap, {"intermediate_proj_out"});
    allReduceNode.outTensorIds =
        atb_speed::common::GetTensorIdxList(tensorMap, {"allreduce_out_proj"});
    ...
    proj_add_biasNode.inTensorIds = atb_speed::common::GetTensorIdxList(
        tensorMap, {"allreduce_out_proj", "in_attn_proj_bias"});
...
atb::Status BuildMLPBlock(const VisionEncoderLayerParam &param,
  ...
  gateUpNode.inTensorIds = atb_speed::common::GetTensorIdxList(
      tensorMap, {"intermediate_post_atten_norm_out", "in_mlp_gate_weight",
                  "in_mlp_gate_bias"});
  ...
  if (isTp) {
    CreateAllReduceOperation(param, &allReduceNode.operation);
    allReduceNode.inTensorIds = atb_speed::common::GetTensorIdxList(
        tensorMap, {"intermediate_down_out"});
    allReduceNode.outTensorIds =
        atb_speed::common::GetTensorIdxList(tensorMap, {"allreduce_down"});
    opGraph.nodes.push_back(allReduceNode);

    CreateAddOperation(&down_add_biasNode.operation);
    down_add_biasNode.inTensorIds = atb_speed::common::GetTensorIdxList(
        tensorMap, {"allreduce_down", "in_mlp_down_bias"});
...
atb::Status EncoderLayer(const VisionEncoderLayerParam &param,
  ...
  opGraph.name = "Qwen2_5_Vision_Encoder_layer";
