...
#include "models/qwen3_vl/qwen3_vl_encoder.h"
...
GetQwen3VL_LayerTensorCandidates() {
...
static std::map<std::string, uint32_t> ConstructTensorMap(
  ...
  auto qwenLayerTensorCandiadates = GetQwen3VL_LayerTensorCandidates();
...
atb::Status BuildQwen3VLAttentionBlock(
    const VisionEncoderLayerParam& param,
...
atb::Status BuildQwen3VLMLPBlock(const VisionEncoderLayerParam& param,
                          const std::map<std::string, uint32_t>& tensorMap,
                          atb::GraphParam& opGraph,
                          bool isTp) {
  ...
  ActivationType act_type = param.MLPActivationType;
...
atb::Status Qwen3VL_EncoderLayer(const VisionEncoderLayerParam& param,
  ...
  atb::GraphParam opGraph;
  opGraph.name = "Qwen3_Vision_Encoder_layer";
  ...
  // Attention
  BuildQwen3VLAttentionBlock(param, tensorMap, opGraph, isTp);
  // MLP
  BuildQwen3VLMLPBlock(param, tensorMap, opGraph, isTp);
