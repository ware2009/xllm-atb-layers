...
std::map<std::string, std::vector<std::string>> GetLatentAttnIntermediateTensorCandidates()
{
	...
	{"o_proj_unpad", {"o_unpad", "o_pass"}},
        {"qk_rope_trans", {"rope_q_t", "rope_k_t"}},
    };
    ...
    uint32_t &inTensorNum, uint32_t &outTensorNum, uint32_t &internalTensorNum)
{
    ...
    if (param.actual_headNum != 0) {
        AddTensorToList(latentAttnIntermediateTensorCandidates, "o_proj_unpad", intermediateTensorList);
    }

    if (param.ropeTrans) {
        AddTensorToList(latentAttnIntermediateTensorCandidates, "qk_rope_trans", intermediateTensorList);
    }
...
void SplitHeadNumHeadDim(const atb::Dims &oldShape, atb::Dims &newShape)
{
    if (oldShape.dimNum == 4) {  // 4: FA
        newShape.dimNum = 5;
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        ...
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        newShape.dims[1] = oldShape.dims[1];  // 1, 1: translatedshapetranslated1translated
        newShape.dims[2] = oldShape.dims[2] / 2;
        newShape.dims[3] = 2;
    } else {
        newShape.dimNum = 3;
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        newShape.dims[1] = oldShape.dims[1] /2;
        newShape.dims[2] = 2;
    }
}

void SqueezeHeadNumHeadDimWithSplit(const atb::Dims &oldShape, atb::Dims &newShape)
{
    if (oldShape.dimNum == 5) {  // 4: FA
        newShape.dimNum = 3;  // 3: translatedshapetranslated3
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        newShape.dims[1] = oldShape.dims[1];  // 1, 1: translatedshapetranslated1translated
        newShape.dims[2] =  oldShape.dims[2] * oldShape.dims[3] * oldShape.dims[4];  // 2, 2, 3: translated
    } else if (oldShape.dimNum == 4) {
        newShape.dimNum = 2;  // 2: translatedshapetranslated2
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        newShape.dims[1] =  oldShape.dims[1] * oldShape.dims[2] * oldShape.dims[3];  // 1, 1, 2: translated
    } else {
        newShape.dimNum = 2;  // 2: translatedshapetranslated2
        newShape.dims[0] = oldShape.dims[0];  // 0, 0: translatedshapetranslated0translated
        newShape.dims[1] =  oldShape.dims[1] * oldShape.dims[2];  // 1, 1, 2: translated
...
{
    if (param.ropeTrans) {
        atb::infer::TransposeParam splitTransQParam;
        if (param.isFA) {
            splitTransQParam.perm = {0, 1, 2, 4, 3};
        } else {
            splitTransQParam.perm = {0, 1, 3, 2};
        }

        atb::Node transposeqQrotNode;
        transposeqQrotNode.inTensorIds = {GetTensorIdx(tensorMap, "rope_q")};
        transposeqQrotNode.outTensorIds = {GetTensorIdx(tensorMap, "rope_q_t")};
        CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(splitTransQParam,
            &transposeqQrotNode.operation));
        transposeqQrotNode.inTensorReshapeFuncs.resize(transposeqQrotNode.inTensorIds.size());
        transposeqQrotNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
            SplitHeadNumHeadDim(oldShape, newShape);
        };
        opGraph.nodes.push_back(transposeqQrotNode);

        atb::infer::TransposeParam splitTransKParam;
        if (param.isFA) {
            splitTransKParam.perm = {0, 1, 3, 2};
        } else {
            splitTransKParam.perm = {0, 2, 1};
        }

        atb::Node transposeqKrotNode;
        transposeqKrotNode.inTensorIds = {GetTensorIdx(tensorMap, "rope_k")};
        transposeqKrotNode.outTensorIds = {GetTensorIdx(tensorMap, "rope_k_t")};
        CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(splitTransKParam,
            &transposeqKrotNode.operation));
        transposeqKrotNode.inTensorReshapeFuncs.resize(transposeqKrotNode.inTensorIds.size());
        transposeqKrotNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
            SplitHeadNumHeadDim(oldShape, newShape);
        };
        opGraph.nodes.push_back(transposeqKrotNode);
    }
    ...
    if (param.ropeTrans) {
        ropeNode.inTensorIds = {
            GetTensorIdx(tensorMap, "rope_q_t"), GetTensorIdx(tensorMap, "rope_k_t"),
            GetTensorIdx(tensorMap, "in_cos_embed"), GetTensorIdx(tensorMap, "in_sin_embed"),
    ...
    if (param.ropeTrans){
        ropeNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
            SqueezeHeadNumHeadDimWithSplit(oldShape, newShape);
        };
...
template <typename NormParamType>
atb::Status AddUnpadSplitNode(const LatentAttentionParam<NormParamType> &param, atb::GraphParam &opGraph,
    std::map<std::string, uint32_t> &tensorMap)
{
    atb::Node splitNode;
    atb::infer::SplitParam splitParam;
    splitParam.splitDim = 1;
    splitParam.splitSizes = {param.actual_headNum, param.selfAttentionParam.headNum - param.actual_headNum };
    CHECK_OPERATION_STATUS_RETURN(atb::CreateOperation(splitParam, &splitNode.operation));

    auto oquant_type = GetLinearQuantType(
        param.denseQuantType == atb_speed::common::PackQuantType::PACK_QUANT_UNDEFINED \
            ? param.packQuantType : param.denseQuantType,
        param.attnLinearQuantType[O_LINEAR_INDEX], false);
    splitNode.inTensorIds = {GetTensorIdx(tensorMap, !param.enableExtraOprojTp ?
        "intermediate_self_attention" : \
        oquant_type == LinearQuantType::LINEAR_W8A8_DEQUANT ? \
        "intermediate_self_attention_padding_quant" : "intermediate_self_attention_padding")};
    splitNode.outTensorIds = {GetTensorIdx(tensorMap, "o_unpad"),
                              GetTensorIdx(tensorMap, "o_pass")};

    splitNode.inTensorReshapeFuncs.resize(splitNode.inTensorIds.size());
    splitNode.inTensorReshapeFuncs[0] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
	if (oldShape.dimNum == 2){
            newShape.dimNum = 3; // 3: x -> [B, N, D]
            newShape.dims[0] = oldShape.dims[0];
            newShape.dims[1] = param.selfAttentionParam.headNum;
            newShape.dims[2] = oldShape.dims[1] / param.selfAttentionParam.headNum; // 2, 3: dim id
        }
	else {
	    newShape = oldShape;
	}
    ...
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap)
{
    ...
    selfOutLinearParallelNode.inTensorIds = {
	    ...
	    param.actual_headNum!=0 ? "o_unpad" : \
	    !param.enableExtraOprojTp ? "intermediate_self_attention" : \
            selfOutLinearParam.fusionLinearParam.quantType == LinearQuantType::LINEAR_W8A8_DEQUANT ? \
                "intermediate_self_attention_padding_quant" : "intermediate_self_attention_padding" ),
...
atb::Status Attention(const LatentAttentionParam<NormParamType> &param, atb::Operation **operation)
{
	...
	if (param.actual_headNum != 0){
	    CHECK_OPERATION_STATUS_RETURN(AddUnpadSplitNode(param, opGraph, tensorMap));
	}
    ...
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap)
{
    ...
    if (param.enablePrefixCache) {
        ...
        // ring cache no mask
        CHECK_OPERATION_STATUS_RETURN(AddGetHistroyNode(param, opGraph, tensorMap));
        ...
        if (param.contextParallelInfo.IsEnabled()) {
            CHECK_OPERATION_STATUS_RETURN(AddKAllGatherCpNode(param, opGraph, tensorMap));
        }
        CHECK_OPERATION_STATUS_RETURN(AddLAttnKProjBNode(param, opGraph, tensorMap));
    ...
    std::map<std::string, uint32_t> &tensorMap)
{
    ...
    selfAttentionNode.inTensorReshapeFuncs[2] = [=](const atb::Dims &oldShape, atb::Dims &newShape) {
        ...
        newShape.dims[0] = oldShape.dims[0];
        newShape.dims[1] = param.selfAttentionParam.headNum;
        newShape.dims[2] = oldShape.dims[1] / param.selfAttentionParam.headNum;  // param.qkNopeHeadDim; // 2: dim id
    };
...
template atb::Status AddQSplitNode(const LatentAttentionParam<atb::infer::RmsNormParam> &param,
    atb::GraphParam &opGraph, std::map<std::string, uint32_t> &tensorMap);
template atb::Status AddGetHistroyNode(const LatentAttentionParam<atb::infer::RmsNormParam> &param,
