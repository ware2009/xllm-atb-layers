...
void DecoderModel::ConstructInternalTensorMap()
{
    ...
    // translatedropetranslatedTensor
    ...
    // translatedadd rmsnormtranslatedtensor
...
uint32_t DecoderModel::CalcWeightTensorSize()
{
    if (this->param.enableKvQuant) {
        this->weightCountPerLayer += 8;  // 8: kv cache int8 translated8translatedinTensor
    }
    if (this->param.enableFA3) {
        this->weightCountPerLayer += 8; // 8: FA3 translated8translatedinTensorensor
    }
    if (this->param.enableReduceQuant) {
        this->weightCountPerLayer += 8;  // 8: lccl reduce int8 translated8translatedinTensor
    }
    if (this->param.enableInterLayerAddNorm || this->param.enableIntraLayerAddNorm) {
        this->weightCountPerLayer += 4;  // 4: addRmsNormQuant translated4translatedinTensor
    }
    if (this->param.normType == LAYER_NORM) {
        this->weightCountFinalNorm = 2;  // 2: LayerNorm translated
    }
    if (this->param.useQKNorm) {
        this->weightCountPerLayer += 2;  // 2: useQKNorm translated2translatedinTensor
    }
...
atb::Status DecoderModel::AddSingleLayer(uint32_t layerId)
{
    ...
    if (this->param.hasAttnDp && this->param.hasMlpTp) {
        ...
        layerNode.outTensors = {layerNode.inTensors.at(weightCountPerLayer)}; // translated
    }
...
atb::Status DecoderModel::AddFinalNorm()
{
    ...
    finalNormNode.outTensors = {finalNormNode.inTensors.at(0)};  // translated
...
atb::Status DecoderModel::AddLmhead()
{
    ...
    // LmHeadtranslated,translatedplaceholdertranslated
...
atb::Status DecoderModel::BindParamHostTensor(uint32_t nodeId)
{
    ...
    if (nodeId != 0) {
        // translatedgraphtranslatedintensortranslatedbindtranslated
        return atb::NO_ERROR;
    }
